                ...
                if (mysqli_stmt_num_rows($stmt) == 1) {                 
                    ...
                    if (mysqli_stmt_fetch($stmt)) {
                        if (password_verify($password, $hashed_password)) {
                            ...
                            logSecurityEvent($conn, 'LOGIN_SUCCESS', $id, 'User successfully authenticated.', $db_email);
                            ...
                            // LOG SECURITY EVENT: FAILED LOGIN (Invalid Password)
                            // User ID is unknown/irrelevant for a failed attempt
                            logSecurityEvent($conn, 'LOGIN_FAILURE', null, 'Invalid password attempt.', $email);
                        }
                    }
                } else {
                    // Email doesn't exist
                    $login_err = "Invalid email or password.";
                    // LOG SECURITY EVENT: FAILED LOGIN (Email Not Found)
                    // User ID is null as the user was not found
                    logSecurityEvent($conn, 'LOGIN_FAILURE', null, 'Email not found attempt.', $email);
                }
            } else {
